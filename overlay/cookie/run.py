"""
Overlay Cookie unit: launch cookie injection and LaunchConfig follow-up flow.
Invoked by driver: python overlay/cookie/run.py <phase>
Reads SIMPRINT_CHROMIUM_ROOT and SIMPRINT_KERNEL_ROOT from environment or
...
KERNEL_ROOT = PROJECT_ROOT.parent.parent  # overlay/cookie -> kernel root


def _read_driver_config(key: str) -> str | None:
    """Read configuration from driver/driver.config."""
...
def _chromium_src() -> Path:
    src = os.environ.get("SIMPRINT_CHROMIUM_ROOT") or _read_driver_config(
        "SIMPRINT_CHROMIUM_ROOT"
    )
    if not src:
        raise SystemExit(
...
def apply_() -> None:
    ...
    for name in order:
        ...
        if not patch_file.is_file():
            continue
        print(f"Applying: overlay/cookie/{name}")
        r = subprocess.run(
            ["patch", "-p1"],
            cwd=chromium_src,
