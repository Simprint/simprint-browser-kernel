"""
Overlay Proxy unit: Simprint IPC proxy configuration + runtime refresh.
Invoked by driver: python overlay/proxy/run.py <phase>
...
KERNEL_ROOT = PROJECT_ROOT.parent.parent  # overlay/proxy -> kernel root
...
def apply_() -> None:
    ...
    for name in order:
        ...
        if not patch_file.is_file():
            continue
        print(f"Applying: overlay/proxy/{name}")
        r = subprocess.run(
            ["patch", "-p1"],
            cwd=chromium_src,
