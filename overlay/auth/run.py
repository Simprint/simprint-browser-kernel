"""
Overlay Auth 单元：启动认证检查 + LaunchConfig IPC 传递
由 driver 调用：python overlay/auth/run.py <phase>
...
KERNEL_ROOT = PROJECT_ROOT.parent.parent  # overlay/auth -> kernel root
...
def apply_() -> None:
    ...
    for name in order:
        ...
        if not patch_file.is_file():
            continue
        print(f"Applying: overlay/auth/{name}")
        r = subprocess.run(
            ["patch", "-p1"],
            cwd=chromium_src,
