"""
Overlay Fingerprint 单元：指纹防护模块（配置系统 + 浏览器指纹伪装）
由 driver 调用：python overlay/fingerprint/run.py <phase>
...
KERNEL_ROOT = PROJECT_ROOT.parent.parent  # overlay/fingerprint -> kernel root
...
def apply_() -> None:
    ...
    for name in order:
        ...
        if not patch_file.is_file():
            continue
        print(f"Applying: overlay/fingerprint/{name}")
        r = subprocess.run(
            ["patch", "-p1"],
            cwd=chromium_src,
