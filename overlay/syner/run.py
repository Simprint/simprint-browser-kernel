"""
Overlay Syner 单元：EventBus IPC 通信 + 输入同步（键盘、鼠标、IME）
由 driver 调用：python overlay/syner/run.py <phase>
...
KERNEL_ROOT = PROJECT_ROOT.parent.parent  # overlay/syner -> kernel root
...
def apply_() -> None:
    ...
    for name in order:
        ...
        if not patch_file.is_file():
            continue
        print(f"Applying: overlay/syner/{name}")
        r = subprocess.run(
            ["patch", "-p1"],
            cwd=chromium_src,
