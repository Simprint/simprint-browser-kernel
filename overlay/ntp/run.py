"""
Overlay NTP 单元：Simprint 新标签页自定义 UI、IP 检测、响应式布局。
由 driver 调用：python overlay/ntp/run.py <phase>
...
KERNEL_ROOT = PROJECT_ROOT.parent.parent  # overlay/ntp -> kernel root
...
def apply_() -> None:
    ...
    for name in order:
        ...
        if not patch_file.is_file():
            continue
        print(f"Applying: overlay/ntp/{name}")
        r = subprocess.run(
            ["patch", "-p1"],
            cwd=chromium_src,
