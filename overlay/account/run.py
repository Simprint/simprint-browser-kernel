"""
Overlay Account 单元：账号自动导入到密码管理器
由 driver 调用：python overlay/account/run.py <phase>
...
KERNEL_ROOT = PROJECT_ROOT.parent.parent  # overlay/account -> kernel root
...
def apply_() -> None:
    ...
    for name in order:
        ...
        if not patch_file.is_file():
            continue
        print(f"Applying: overlay/account/{name}")
        r = subprocess.run(
            ["patch", "-p1"],
            cwd=chromium_src,
